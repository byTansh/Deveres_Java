...
public class SuperHeroiController {
    ...
    @PostMapping("/heroi")
    public ResponseEntity<?> saveProduct(@RequestBody Heroi entity) {
        Heroi cadastroHeroi;

        try{
            cadastroHeroi =  heroiRepository.save(entity);
        } catch(Exception e) {
            return new ResponseEntity<String>("Erro ao cadastrar heroi", HttpStatus.BAD_REQUEST);
        }

        return new ResponseEntity<Heroi>(cadastroHeroi, HttpStatus.OK);
    }
